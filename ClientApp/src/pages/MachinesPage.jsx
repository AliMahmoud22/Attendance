...
function StatCard({ label, value, accent, lightColor }) {
  ...
  return (
    <div
      className={`rounded-2xl p-4 border ${lightColor}  dark:bg-linear-to-r from-white dark:from-gray-800 from-10% ${accent} to-white to-90% dark:to-gray-800  border-gray-200 dark:border-gray-700 shadow-sm`}
    >
      <p className="text-xs font-semibold uppercase text-gray-300 ">{label}</p>
      <p className="text-2xl sm:text-3xl font-bold text-gray-300 ">{count}</p>
    </div>
  );
}
  ...
  return (
    <span
      className={`px-4 py-2 rounded-full text-xs font-semibold ${
        enabled ? "bg-green-200 text-green-700" : "bg-red-200 text-red-600"
      }`}
    >
...
export default function MachinesPage() {
  ...
  /* ── Load machines ── */
  const loadMachines = async (p = 1, replace = true, customParams = {}) => {
    try {
      if (p === 1) setLoading(true);

      const params = new URLSearchParams({
        page: p,
        name: customParams.name ?? search,
        status: customParams.status ?? statusFilter,
      });
  ...
  const handleDelete = async (machine) => {
    try {
      const res = await apiFetch(`/api/machines/permanent/${machine.id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      showToast(data.message);
      ...
      <div className="min-h-screen p-4 sm:p-6 bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          <StatCard
            label="Total"
            value={totalMachines}
            lightColor={"bg-[#124170]"}
            accent="via-[#124170]"
          />
          <StatCard
            label="Active"
            value={activeMachines}
            lightColor={"bg-[#016B61]"}
            accent="via-[#016B61]"
          />
          <StatCard
            label="Disabled"
        ...
        >
          ...
          <button
            ...
            onClick={() => {
              ...
              setSearch(resetValues.name);
              setStatus(resetValues.status);
              loadMachines(1, true, resetValues);
            }}
          ...
          {!loading &&
            machines.map((m, i) => (
              ...
              >
                ...
                <span className="sm:col-span-1 text-lg ">
                  {m.machineNumber}
                </span>
                ...
                <div className="sm:col-span-3  justify-center flex gap-2">
                  ...
                  <button
                    onClick={() => setModal({ type: "disable", machine: m })}
                    className="px-2 py-1 btn-action text-md rounded bg-[#393e49] text-white"
                  >
                    Disable
                  </button>
                  <button
